...
static void load_bg() {
  ...
  // Increment snooze counters to account for initial loop around and make sure we really snooze for right minute time
  ...
  //APP_LOG(APP_LOG_LEVEL_DEBUG, "LOW & HIGH SNOOZE VALUE IN: %i %i", LOW_SNZ_MIN, HIGH_SNZ_MIN);
...
static void load_battlevel() {
  APP_LOG(APP_LOG_LEVEL_INFO, "LOAD BATTLEVEL, FUNCTION START");
    ...
    APP_LOG(APP_LOG_LEVEL_DEBUG, "LOAD BATTLEVEL, BATTLEVEL WITH PERCENT: %s", battlevel_percent);
